...
Partial Class NtsEd
    ...
    Public Property IdNts() As Integer
        Get
            Return hdnIdNts.Value
        End Get
        Set(ByVal value As Integer)
            hdnIdNts.Value = value
    ...
    Private Sub Save()
        Using claseN As New DataClassesDataContext
            Dim objN As New NT
            If IdNts > 0 Then objN = (From c In claseN.NTs Where c.IdNts = IdNts Select c).Single
            ...
            If DateControlPRCHS_DATE.EsValido Then objN.PRCHS_DATE = DateControlPRCHS_DATE.Fecha Else objN.PRCHS_DATE = Nothing
            objN.REMARKS = txtRemarks.Text
            objN.ROOMS = txtRooms.Text

            If DateControlSaleDate.EsValido Then objN.SALE_DATE = DateControlSaleDate.Fecha Else objN.SALE_DATE = Nothing
            objN.SALE_TIME = txtTime.Text
            objN.SITE = txtSite.Text
            objN.SQFT = txtSqFt.Text
            objN.STORY = txtStory.Text

            objN.T_PHONE = MaskT_PHONE.Value '.ToString.Replace(" ", "").Replace("-", "")
            ...
            If IdNts = 0 Then
                claseN.NTs.InsertOnSubmit(objN)
                objN.IdStatus = 1
                objN.Created = Now
            Else
                'objN.IdStatus = 3
    ...
    Private Sub Fill()
        Clean()
        Using claseN As New DataClassesDataContext
            Dim objN = (From c In claseN.NTs Where c.IdNts = IdNts Select c).Single
            ...
            If Not objN.T_S_NO Is Nothing Then txtSaleNo.Text = objN.T_S_NO
            If Not objN.ADDRESS Is Nothing Then txtAddress.Text = objN.ADDRESS : lblAddress.Text = objN.ADDRESS
            If Not objN.AMOUNT Is Nothing Then txtMidBid.Text = objN.AMOUNT
            ...
            If Not objN.NTS Is Nothing Then txtNTS.Text = objN.NTS
            ...
            If Not objN.SALE_DATE Is Nothing Then DateControlSaleDate.Fecha = objN.SALE_DATE
            If Not objN.SALE_TIME Is Nothing Then txtTime.Text = objN.SALE_TIME
            If Not objN.SITE Is Nothing Then txtSite.Text = objN.SITE
            ...
            'txtTD1.Text = IIf(objN.TD1 Is Nothing, "", objN.TD1)
            If Not objN.TD2 Is Nothing Then txtTD2.Text = objN.TD2 Else txtTD2.Text = ""
            ...
            If Not objN.TD3 Is Nothing Then txtTD3.Text = objN.TD3 Else txtTD3.Text = ""
            ...
            If Not objN.TD4 Is Nothing Then txtTD4.Text = objN.TD4 Else txtTD4.Text = ""
            ...
            If Not objN.TD5 Is Nothing Then txtTD5.Text = objN.TD5 Else txtTD5.Text = ""
            ...
            If Not objN.TD6 Is Nothing Then txtTD6.Text = objN.TD6 Else txtTD6.Text = ""
            ...
            'If Not objN.IdStatus Is Nothing Then 
            ddlStatus.SelectedValue = objN.IdStatus
    ...
    Private Sub Counter()
        Using clase As New DataClassesDataContext
            Dim lista = DataFunctions.SelectNtsData()
            '(From c In clase.NTs Where (c.ADDRESS.Contains(Session("txtAddress").ToString) Or c.ADDRESS Is Nothing) _
            ...
            '      Select c Order By IdNts Descending).ToList
            Dim ListaNTS As New ArrayList
            For i As Integer = 0 To lista.Count - 1
                ListaNTS.Add(lista(i).IdNts)
                If lista(i).IdNts = IdNts Then Index = i
            Next
            Session("ListaNTS") = ListaNTS
    ...
    Protected Sub btnDel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnDel.Click
        Dim IdDel As Integer = IdNts
        Clean()
        IdNts = 0
        Dim allNts = DataFunctions.SelectNtsData()

        Dim objNP = (From c In allNts Where c.IdNts > IdDel Order By c.IdNts Select c).ToList
        If objNP.Count > 0 Then
            IdNts = objNP(0).IdNts
        Else
            objNP = (From c In allNts Where c.IdNts < IdDel Order By c.IdNts Descending Select c).ToList
            If objNP.Count > 0 Then
                IdNts = objNP(0).IdNts
            End If
        End If
    ...
    Protected Sub btnGoNext_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnGoNext.Click
        Dim ListaNTS As ArrayList = Session("ListaNTS")
        Index += 1
        If Index > ListaNTS.Count - 1 Then Index = ListaNTS.Count - 1
        IdNts = ListaNTS(Index)
        Fill()
    End Sub

    Protected Sub btnGoLast_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnGoLast.Click
        Dim ListaNTS As ArrayList = Session("ListaNTS")
        Index = ListaNTS.Count - 1
        IdNts = ListaNTS(Index)
        Fill()
    End Sub
    ...
    Protected Sub btnGo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnGo.Click
        If IsNumeric(txtId.Text) Then
            Dim ListaNTS As ArrayList = Session("ListaNTS")
            If ListaNTS.IndexOf(CInt(txtId.Text)) > -1 Then
                Index = ListaNTS.IndexOf(CInt(txtId.Text))
                IdNts = ListaNTS(Index)
                Fill()
            End If
        End If
